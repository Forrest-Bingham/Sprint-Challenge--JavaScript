...
let universities = [];
/*
 universities = graduates.map(function(item){
  return `${item.university}`;
});*/

universities = graduates.map((item) => item.university);
...
/*contactInfo = graduates.map(function(item){
  return `${item.first_name} ${item.email}`;
});*/

contactInfo = graduates.map((item) =>  `${item.first_name} ${item.email}`);
